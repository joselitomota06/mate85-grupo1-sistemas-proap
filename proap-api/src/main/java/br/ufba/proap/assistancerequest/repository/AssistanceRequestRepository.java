...
public interface AssistanceRequestRepository extends JpaRepository<AssistanceRequest, Long> {
	...
	@Query("SELECT ar.id, ar.valorAprovado, ar.createdAt, ar.dataAprovacao, ap.name FROM AssistanceRequest ar LEFT JOIN ar.avaliadorProap ap WHERE DATE(ar.createdAt) BETWEEN :startDate AND :endDate AND ar.situacao = 1 ")
	...
	@Query("SELECT ar.id, ar.valorAprovado, ar.dataAprovacao, ap.name, ar.custoFinalCeapg, ar.observacoesCeapg, ac.name FROM AssistanceRequest ar LEFT JOIN ar.avaliadorProap ap LEFT JOIN ar.avaliadorCeapg ac WHERE DATE(ar.dataAprovacao) BETWEEN :startDate AND :endDate AND ar.situacao = 1")
	List<Object[]> findAllCeapgRequests(LocalDate startDate, LocalDate endDate);

	@Query("SELECT ar.id, ar.valorAprovado, ar.dataAprovacao, ap.name, ar.custoFinalCeapg, ar.observacoesCeapg, ac.name FROM AssistanceRequest ar LEFT JOIN ar.avaliadorProap ap LEFT JOIN ar.avaliadorCeapg ac WHERE DATE (ar.dataAprovacao) BETWEEN :startDate AND :endDate AND ar.situacao = 1 AND ar.avaliadorCeapg IS NULL")
