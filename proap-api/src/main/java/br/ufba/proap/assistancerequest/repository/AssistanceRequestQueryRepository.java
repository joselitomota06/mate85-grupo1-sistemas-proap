...
public class AssistanceRequestQueryRepository {
    ...
    public List<AssistanceRequestDTO> findFiltered(String prop, boolean ascending, int page, int limitPerPage, User user) {
        StringBuilder query = new StringBuilder("from AssistanceRequestDTO ar");

        if(user != null) query.append(" where ar.user.id = :userId");
        ...
        if(user != null) typedQuery.setParameter("userId", user.getId());
